...
import ClientConvo3 from './ClientConvo3';
...
async function readConversationYaml(): Promise<ConversationSpec> {
  const candidates = [
    path.join(process.cwd(), 'app', 'app', 'demo', 'convo3', 'conversation.yaml'),
    path.join(process.cwd(), 'app', 'demo', 'convo3', 'conversation.yaml'),
    path.join(process.cwd(), 'demo', 'convo3', 'conversation.yaml'),
  ];
  ...
  throw new Error('conversation.yaml not found in convo3 folder');
}
