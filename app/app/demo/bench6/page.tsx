...
/* ---------- Demo YAML (Benchmark 6) ---------- */
const DEMO_YAML = `
version: 1
name: "Projects (expandable list)"
widgets:
  - id: projects
    type: list
    label: "Projects"
...
type ListItemSpec = { expandable?: boolean; fields: (TextField | SelectField)[] };
type ListWidget = {
  id: string;
  type: 'list';
  label: string;
...
type Widget = ListWidget; // only `list` on this page
type Template = {
  version: number;
  name: string;
  widgets: Widget[];
...
/* ---------- Component ---------- */
export default function Bench6Page() {
  ...
  async function handleSeed() {
    const createdTpl = await api<{ id: string; yaml: string }>('/templates', {
      method: 'POST',
      body: JSON.stringify({ yaml: yamlText, name: 'Bench6' }),
    });
    setTplId(createdTpl.id);

    // Seed projects exactly like your example
  ...
  function renderList(w: ListWidget) {
    ...
    const isExpandable = !!w.item.expandable;
            ...
            <div key={base} style={{ border: '1px solid #eee', borderRadius: 8, padding: 10, marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, opacity: 0.6 }}>id={id} · base={base}</div>
                </div>
                {isExpandable && (
                  <button
              ...
              {!isExpandable || open ? (
                <div style={{ marginTop: 8 }}>
                  {w.item.fields.map((f) => {
                    const fieldPath = joinPointer(base, f.binding.path);
  ...
  return (
    <main style={{ maxWidth: 900 }}>
      <h1>Benchmark 6 — Expandable list · edit Project B status</h1>
