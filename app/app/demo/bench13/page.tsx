...
import ClientBench13 from './ClientBench13';
...
export default async function Page() {
  const candidates = [
    path.join(process.cwd(), 'app', 'app', 'demo', 'bench13', 'template.yaml'),
    path.join(process.cwd(), 'demo', 'bench13', 'template.yaml'),
    path.join(process.cwd(), 'app', 'demo', 'bench13', 'template.yaml'),
  ];
  const yamlText = await readFirst(candidates);
  const template: Template = loadYamlTemplateFromString(yamlText);
  return <ClientBench13 template={template} />;
}
