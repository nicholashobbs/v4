...
import ClientBench12 from './ClientBench12';
...
export default async function Page() {
  const candidates = [
    path.join(process.cwd(), 'app', 'app', 'demo', 'bench12', 'template.yaml'),
    path.join(process.cwd(), 'demo', 'bench12', 'template.yaml'),
    path.join(process.cwd(), 'app', 'demo', 'bench12', 'template.yaml'),
  ];
  const yamlText = await readFirst(candidates);
  const template: Template = loadYamlTemplateFromString(yamlText);
  return <ClientBench12 template={template} />;
}
